...
formateMain.prototype = {
   ...
   * @returns {undefined}
   */
  dateFormatByString: function (date) {
  ...
  weekFormat: function (date) {
    ...
    if (typeof (date) == 'string') {
      date = new Date(Number(date.substr(0, 4)), Number(date.substr(4, 2)), Number(date.substr(6, 2)))
    }
    //console.log(date);
    return week[date.getDay()];
  },
  weekFormats: function (date) {
    var week = ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"];
    //console.log(date);
    if (typeof (date) == 'string') {
  ...
  dateFormat: function (d, type, f) {
     ...
     * f: 返回值格式，'YY-MM-DD'为年-月-日，'MM-DD'为月-日；
     */
